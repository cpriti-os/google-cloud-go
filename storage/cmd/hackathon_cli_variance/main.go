...
func main() {
	...
	fmt.Println("==================================================================")
	fmt.Println(" 🚀 6-MINUTE HIGH-VARIANCE WORKLOAD (TARGET: cpriti-sdk-autotune-bench)")
	fmt.Println("==================================================================")
	...
	// RUN 1: UNTUNED
	fmt.Println("\n❌ STARTING: STANDARD GO SDK (Untuned | Static 16MB Chunks)")
	fmt.Println("------------------------------------------------------------------")
	baseMB, _ := runWorkload(false, phases)

	// RUN 2: TUNED
	fmt.Println("\n✅ STARTING: ADAPTIVE AI SDK (Tuned | PCU + EWMA Dynamic)")
	fmt.Println("------------------------------------------------------------------")
...
func runWorkload(useTuner bool, phases []Phase) (float64, float64) {
	...
	if useTuner {
		...
		cfg.MaxMemoryBudget = 512 * 1024 * 1024 // Allow PCU up to 512MB
		w = bucket.Object("variance_tuned.bin").NewAdaptivePCUWriter(ctx, cfg)
	} else {
		bw := bucket.Object("variance_base.bin").NewWriter(ctx)
	...
	for _, p := range phases {
		...
		// Attempt to push data at the TargetMBps rate for exactly 30 seconds
		timer := time.Now()
	...
	PhaseLoop:
		for {
			...
			case <-phaseTimeout:
				break PhaseLoop
			default:
				// Push 1MB blocks to allow granular throttling
				bufSize := 1 * 1024 * 1024
				...
				// Calculate delay to maintain application target rate
				...
				if drift < 0 { 
					// Running too fast! Throttle the application to simulate target MB/s
				...
				if useTuner && bytesInPhase == int64(bufSize * 5) {
					if p.TargetMBps >= 150 {
						fmt.Printf("   >> 🤖 AI: EWMA detected burst %d MB/s -> Scaling Chunk to 64MB & Launching workers\n", p.TargetMBps)
	...
	w.Close() // wait for PCU limits to compose
