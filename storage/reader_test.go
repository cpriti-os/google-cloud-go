...
func TestReaderExtraBytesLogging(t *testing.T) {
	...
	// 12 bytes read, but remain was 5. So remain is now -7.
	// Log should be triggered.
	if n != 12 {
	...
	// Next read shouldn't log again because remain is set to -1
	logOutput.Reset()
	_, _ = r.Read(buf)
	if logOutput.Len() > 0 {
		t.Errorf("expected no additional logging, but got: %q", logOutput.String())
	}
}
...
func TestReaderWriteToExtraBytesLogging(t *testing.T) {
	...
	// 12 bytes copied, but remain was 5. Log should be triggered.
	if n != 12 {
